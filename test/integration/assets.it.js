...
describe('/assets endpoint', () => {
  ...
  it('should post', async () => {
    ...
    const created = await request(server).post('/assets').set("x-api-key", "ky").send(assetParams);
  ...
  it('should validate asset type exists before post', async () => {
    ...
    const result = await request(server).post('/assets').set("x-api-key", "ky").send(assetParams);
